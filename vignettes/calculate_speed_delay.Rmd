...
knitr::opts_chunk$set(
  ...
  fig.width = 8,
  fig.height = 4,
  fig.path = "../man/figures/calculate_speed_delay-"
)
...
```{r}
#> 10min speed summary
summary(agg_ten$speed.calc)
#> Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#> 0.50   54.93   61.98   60.78   68.22  166.91   37882 

#> 10 minute speed NA percentage
summary(agg_ten$speed.calc)[7] / nrow(agg_ten)
#> NA's 
#> 0.329661 


#> 1 hour speed summary
summary(agg_hour$speed.calc)
#> Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#> 1.885  54.950  61.352  60.435  67.131  91.054    6088 

#> 1 hour minute speed NA percentage
summary(agg_hour$speed.calc)[7] / nrow(agg_hour)
#> NA's 
#> 0.317878 
```
...
cowplot::plot_grid(
  agg_ten %>%
    ...
    summarise(
      pc_n = sum(is.na(speed.calc)) / length(speed.calc),
      .groups = "keep"
    ) %>%
    ggplot() +
    geom_col(
...
```
```{r, include=FALSE}
ggsave(device = "png", dpi = 72, filename = "img/plot_speed_nulls.png")
```
![](img/plot_speed_nulls.png)  
...
plot_ten <- ggplot() +
  ...
  scale_x_continuous("Speed (mph)", limits = range(agg_ten$speed.calc)) +
  theme_minimal() +
...
```
```{r, include=FALSE}
ggsave(device = "png", dpi = 72, filename = "img/plot_occupancy_speed_volume.png")
```
...
rf_imp_day_type <- mice(agg_hour,
  method = "rf",
  formulas = c(
    as.formula("speed.calc ~ volume.sum + occupancy.sum + interval_bin + day_type")
  ),
  maxit = 5, m = 5, print = F
)
tictoc::toc()
...
agg_hour_imputed <- map_dfr(seq(1, 6), .f = function(x) {
  all_imps[[x]] %>%
    mutate(imp_n = x) %>%
    select(interval_bin, speed.calc, day_type, imp_n)
})
```
...
ggplot() +
  ...
  labs(
    x = "Speed",
    y = "Density",
    title = "Imputed and original speed.calc density by day type",
  ...
  theme_minimal()
```
![](img/plot_speed_imputation_density.png)
```{r, include=FALSE}
ggsave(device = "png", dpi = 72, filename = "img/plot_speed_imputation_density.png")
```
...
ggplot() +
  ...
  theme_minimal()
```
![](img/plot_speed_imputation_hourly.png)
```{r, include=FALSE}
ggsave(device = "png", dpi = 72, filename = "img/plot_speed_imputation_hourly.png")
```
## Distance  
...
time_test <- complete(rf_imp_day_type) %>% 
  ...
  mutate(travel_time_speed = dhours(distance/speed.calc),
...
ref_speed <- time_test %>% 
  ...
  summarize(ref_time_speed = mean(travel_time_speed),
            ref_time_limit = mean(travel_time_limit),
            ref_speed = median(speed.calc),
            .groups = "keep") %>% 
  unique()
...
ggplot() +
  ...
  theme_minimal()
```
![](img/plot_delay_hour_speed.png)
```{r, include=FALSE}
ggsave(device = "png", dpi = 72, filename = "img/plot_delay_hour_speed.png")
```
